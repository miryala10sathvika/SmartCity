...
def post_to_onem2m(sensor_name, timestamp, sensor1_data, sensor2_data, sensor1_location, sensor2_location):
    ...
    if response.status_code == 201:
        print(f"Data posted successfully for {sensor_name} at {timestamp}. Current time: {current_time}")
    else:
        print(f"Failed to post data for {sensor_name}. Status code: {response.status_code}")
...
def create_subscription(sensor_type, notification_url):
    ...
    if response.status_code == 201:
        ...
        print(f"Failed to create subscription for {sensor_type}. Status code: {response.status_code}")
    ...
    while True:
        try:
            for file_path in file_paths:
                ...
                if last_modified_times[file_path] is None or current_modified_time != last_modified_times[file_path]:
